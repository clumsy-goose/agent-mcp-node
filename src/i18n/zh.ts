const zh = {
  ...
  "auth.gen.title": "JWT 生成器",
  "auth.gen.hint": "用私钥在浏览器本地签发一个 JWT，便于在本地 dev 直接验证鉴权链路。私钥仅用于本地测试，切勿在生产前端暴露。",
  "auth.gen.alg": "算法",
  "auth.gen.key": "私钥 / 密钥 (PEM)",
  "auth.gen.keyPlaceholder": "-----BEGIN PRIVATE KEY----- …（RS256 需 PKCS8 格式）",
  "auth.gen.keyFromEnv": "已从环境变量 VITE_MAKERS_JWT_PRIVATE_KEY 读取私钥",
  "auth.gen.pkcs8Hint": "RS256 要求 PKCS8 格式私钥（-----BEGIN PRIVATE KEY-----）。PKCS1（RSA PRIVATE KEY）不被浏览器 Web Crypto 支持。",
  "auth.gen.sub": "sub（用户标识）",
  "auth.gen.exp": "exp（过期时间，本地时区）",
  "auth.gen.generate": "生成并填入 token",
  "auth.gen.error": "生成失败：",
  "auth.gen.success": "生成成功",
  "auth.gen.copy": "复制",
  "auth.gen.copied": "已复制",
  "auth.gen.filled": "该 token 已同时填入上方 JWT Token 框并保存，可直接用于 /whoami 探测。",
  "auth.gen.errorEmptyKey": "私钥 / 密钥为空",
  "auth.gen.errorEmptySub": "sub 不能为空",
  "auth.gen.errorExp": "过期时间必须晚于当前时间",
